...
class WC_PAYLER extends WC_Payment_Gateway
{
  ...
  public function __construct()
  {
    ...
    $this->fiscalshipping = $this->get_option('fiscalshipping');
  ...
  {
    if (!in_array(get_option('woocommerce_currency'), ['RUB', 'EUR', 'USD', 'INR'])) {
      return false;
    }
    return true;
  }
...
?>
    ...
    <?php else : ?>
      <div class="inline error">
        <p><strong><?php _e('GATEWAY is off', 'woocommerce'); ?></strong>: <?php _e('PAYLER doesn\' support your shop currency.', 'woocommerce'); ?></p>
      </div>
<?php
    endif;
  } // End admin_options()
  ...
  function init_form_fields()
  {
    $this->form_fields = array(
      ...
      'testmail' => array(
        ...
        'type' => 'text',
        'description' => __('Orders for given email will trigger 1 RUB/USD/EUR bills.', 'woocommerce'),
        'default' => ''
      ),
      'description' => array(
      ...
      'fiscalshipping' => array(
        'title' => __('Shipping title', 'woocommerce'),
        'type' => 'text',
        'description' => __('Shipping description in fiscal check.', 'woocommerce'),
  ...
  public function kkt($Data)
  {
    ...
    // shipping row
    $shipping_amount = $order->get_shipping_total('edit');
    if ($shipping_amount > 0) {
      $kkt_item = array(
        'name' => $this->fiscalshipping,
        'quantity' => 1,
        'sum' => number_format($shipping_amount, 2),
        'price' => number_format($shipping_amount, 2),
        'payment_method' => 'full_prepayment',
        'payment_object' => 'commodity',
        'vat' => (object) ['type' => $this->saletax],
      );
